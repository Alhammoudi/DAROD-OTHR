...
def anchors_generation(config, train=True):
    ...
    cc = np.array([[2.87643672, 35.54090354],
                   [12.47214148, 13.874603],
                   [64.73229509, 3.5706785],
                   [3.05321095, 53.58569724],
                   [25.95271822, 2.05351839]])
    w, h = cc[:, 0] / config["model"]["input_size"][1], cc[:, 1] / config["model"]["input_size"][0]
...
def generate_delta_map(bboxes, gt_boxes):
    ...
    bboxes = denormalize_bboxes(bboxes,535, 577)
    gt_boxes = denormalize_bboxes(gt_boxes, 535, 577)
    #
    bbox_y1, bbox_x1, bbox_y2, bbox_x2 = tf.split(bboxes, 4, axis=-1)
    gt_y1, gt_x1, gt_y2, gt_x2 = tf.split(gt_boxes, 4, axis=-1)
