...
import sys
sys.path.append("C:/Users/oalha/Desktop/darod/darod-main")
from darod.datasets.othr_builder import othr as othr_module
...
#CHANGED
def get_dataset(name, split, data_dir, version=None):
    if name == "othr":
        builder = othr_module.Othr(data_dir=data_dir)
        builder.download_and_prepare()  # Will be skipped if already prepared
        return builder.as_dataset(split=split), builder.info
    else:
        return tfds.load(name, split=split, data_dir=data_dir, with_info=True)
