                ...
                if start_idx <= idx <= end_idx:
                    if layout == "2D":
                        fig = viz_utils.showCameraRD(camera_image=images[0], rd_spectrum=spectrums[0],
                                                     ...
                                                     gt_boxes=gt_boxes[0], gt_labels=gt_labels[0], dataset=dataset,
                    ...
                    print(f"Saved image: pred_{idx}.png")  # Debug: Confirm image saved
                    plt.clf()
                idx += 1
                if idx >= end_idx:
                    break

    import imageio.v2 as imageio  # Update import to avoid deprecation warning

    png_dir = r'C:\\Users\\oalha\\Desktop\\darod\\images'
    images = []
    for file_name in sorted(os.listdir(png_dir)):
        print(f"Found file: {file_name}")  # Debug: Check files in directory
